...
mod tests {
    ...
    fn test_add_edge() {
        ...
        graph.add_edge("1".to_string(), "4".to_string());
        graph.add_edge("1".to_string(), "4".to_string());
        graph.add_edge("2".to_string(), "4".to_string());
        graph.add_edge("3".to_string(), "4".to_string());
        graph.add_edge("3".to_string(), "4".to_string());
        graph.add_edge("4".to_string(), "4".to_string());
        graph.add_edge("5".to_string(), "4".to_string());
        graph.add_edge("6".to_string(), "4".to_string());
        graph.add_edge("7".to_string(), "4".to_string());
        graph.add_edge("7".to_string(), "4".to_string());
        graph.add_edge("8".to_string(), "4".to_string());

        assert_eq!(graph.0.len(), 3);
        assert_eq!(graph.0.get("A"), Some(&vec!["B".to_string(), "C".to_string()]));
        assert_eq!(graph.0.get("B"), Some(&vec!["D".to_string()]));
        assert_eq!(graph.0.get("C"), Some(&vec!["E".to_string()]));
    }
    ...
    fn test_bfs_shortest_path() {
        ...
        assert_eq!(bfs_shortest(&graph.0, "A", "E"), Ok(2));
        assert_eq!(bfs_shortest(&graph.0, "A", "D"), Ok(1));
        assert_eq!(bfs_shortest(&graph.0, "B", "E"), Ok(1));
        assert_eq!(bfs_shortest(&graph.0, "C", "D"), Ok(2));
    }

    #[test]
    fn test_read_graph_from_file() {
        let result = read_graph_from_file("graph.txt", 5);
        assert!(result.is_ok());

        let graph = result.unwrap();
        assert_eq!(graph.0.len(), 4);
        assert_eq!(graph.0.get("A"), Some(&vec!["B".to_string(), "C".to_string()]));
        assert_eq!(graph.0.get("B"), Some(&vec!["D".to_string()]));
        assert_eq!(graph.0.get("C"), Some(&vec!["E".to_string()]));
    }
